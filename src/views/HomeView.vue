  ...
  <div class="p-4 sm:p-8 max-w-6xl mx-auto print-hide w-full overflow-y-auto">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <section class="bg-white p-6 rounded-none shadow-sm border border-gray-200 block">
        ...
        <div class="bg-gray-50 border border-gray-200 p-5 rounded-none mb-6 space-y-4 shadow-inner">
          <label class="text-sm font-extrabold text-gray-800 uppercase tracking-wider block">Create New Team</label>
          <input v-model="newTeamName" placeholder="Team Name..." class="border border-gray-300 rounded-none px-3 py-2 w-full outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-400" />

          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label class="text-[11px] font-bold uppercase text-gray-500 mb-2 block">Team Color</label>
              <div class="flex flex-wrap gap-2">
                <button v-for="c in colors" :key="c" @click="selectedColor = c" :class="[c, 'w-6 h-6 rounded-none cursor-pointer border-2 transition', selectedColor === c ? 'border-gray-800 scale-110 shadow-sm' : 'border-transparent']" :title="c"></button>
              </div>
            </div>
            <div>
              <label class="text-[11px] font-bold uppercase text-gray-500 mb-2 block">Mascot Icon</label>
               <div class="flex flex-col gap-2">
                 <div class="flex flex-wrap gap-1">
                  <button v-for="i in icons" :key="i" @click="selectedIcon = i" :class="['w-7 h-7 rounded-none flex items-center justify-center cursor-pointer transition', selectedIcon === i ? 'bg-gray-300 shadow-inner' : 'hover:bg-gray-200']" :title="i">
                    <component :is="(LucideIcons as any)[i]" class="w-4 h-4 text-gray-800"/>
                  </button>
                  <button @click="selectedIcon = 'custom'" :class="['w-7 h-7 rounded-none flex items-center justify-center cursor-pointer transition text-[9px] font-black text-gray-800 uppercase', selectedIcon === 'custom' ? 'bg-gray-300 shadow-inner' : 'hover:bg-gray-200']" title="Custom URL">URL</button>
                 </div>
                 <input v-if="selectedIcon === 'custom'" v-model="customIconUrl" placeholder="Image URL (http://...)" class="border border-gray-300 rounded-none px-2 py-1.5 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-400 text-xs w-full" />
               </div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
             <div class="flex flex-col">
               ...
               <select v-model="selectedMatchType" @change="onMatchTypeChange" class="border border-gray-300 rounded-none px-2 py-1.5 text-sm bg-white outline-none focus:border-blue-500">
             ...
             <div class="flex flex-col">
               ...
               <select v-model="selectedDefaultFormationId" class="border border-gray-300 rounded-none px-2 py-1.5 text-sm bg-white outline-none focus:border-blue-500">
        ...
        <ul class="space-y-2">
          <li v-if="store.teams.length === 0" class="text-gray-400 text-sm py-8 text-center italic border-2 border-dashed rounded-none">No teams created.</li>
          <li v-for="team in store.teams" :key="team.id" class="flex justify-between items-center p-3 hover:bg-gray-50 border rounded-none transition duration-150 shadow-sm bg-white">
            <div class="flex items-center space-x-4">
              <div :class="[team.color, 'w-10 h-10 rounded-none flex items-center justify-center text-white shrink-0 shadow-inner overflow-hidden']">
            ...
            <div class="flex items-center space-x-2">
              <router-link :to="`/team/${team.id}`" class="text-blue-600 hover:bg-blue-50 px-3 py-1.5 rounded-none border border-transparent hover:border-blue-100 text-sm font-medium transition">Roster</router-link>
              <button @click="store.deleteTeam(team.id)" class="text-gray-400 hover:text-red-600 hover:bg-red-50 p-2 rounded-none transition" title="Delete Team"><Trash2 class="w-4 h-4"/></button>
            </div>
          </li>
        </ul>
      </section>
    ...
    <div class="mt-8 bg-white p-6 rounded-none shadow-sm border border-gray-200">
       <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 space-y-3 sm:space-y-0 border-b border-gray-100 pb-4">
          ...
          <router-link to="/builder" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-none shadow-sm transition flex items-center gap-2 text-sm uppercase tracking-wide">
             <component :is="LucideIcons.Plus" class="w-4 h-4"/> Create Formation
          </router-link>
       </div>
       
       <ul class="space-y-3">
          <li v-if="store.customFormations.length === 0" class="text-gray-400 text-sm py-8 text-center italic border-2 border-dashed rounded-none bg-gray-50">No custom formations created.</li>
          <li v-for="formation in store.customFormations" :key="formation.id" class="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 hover:bg-gray-50 border rounded-none transition duration-150 shadow-sm bg-white space-y-3 sm:space-y-0">
            <div>
            ...
            <div class="flex items-center space-x-2 w-full sm:w-auto">
               <button @click="previewFormation(formation)" class="bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 font-bold px-4 py-2 rounded-none text-sm transition shadow-sm flex-1 sm:flex-none">Preview</button>
               <button @click="deleteFormation(formation.id)" class="text-gray-400 hover:text-red-600 hover:bg-red-50 p-2 rounded-none transition" title="Delete Formation"><Trash2 class="w-4 h-4"/></button>
            </div>
          </li>
       </ul>
    </div>
    ...
    <div v-if="previewNode" class="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm" @click.self="previewNode = null">
      <div class="bg-white rounded-none shadow-2xl p-6 w-full max-w-lg flex flex-col max-h-[90vh]">
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-2xl font-black text-gray-800 tracking-tight">{{ previewNode.name }} <span class="bg-gray-100 text-gray-500 text-[10px] uppercase font-bold px-2 py-0.5 rounded-none ml-2 align-middle">{{ previewNode.type }}</span></h2>
          <button @click="previewNode = null" class="p-2 text-gray-400 hover:bg-gray-100 rounded-none transition" aria-label="Close Preview"><component :is="LucideIcons.X" class="w-5 h-5"/></button>
        </div>
        <div class="flex-1 overflow-auto bg-gradient-to-br from-gray-100 to-gray-200 rounded-none p-4 sm:p-8 flex items-center justify-center relative">
            <div class="w-full max-w-[400px] aspect-[3/4] relative bg-[#2a8b3e] rounded-none border-[6px] border-white shadow-[0_10px_30px_rgba(0,0,0,0.15)] overflow-hidden">
                <div class="absolute inset-0 pattern-grass"></div>
                <div class="absolute inset-0 m-4 border-2 border-white/60 pointer-events-none rounded-none"></div>
